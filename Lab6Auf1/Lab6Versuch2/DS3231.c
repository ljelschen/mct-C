...
void SetAlarm2Rate(uint8_t alarm1Rate){
	...
	uint8_t A2M1 = getBitFromRegister(alarm1Rate, 0) << 7;
	uint8_t A2M2 = getBitFromRegister(alarm1Rate, 1) << 7;
	uint8_t A2M3 = getBitFromRegister(alarm1Rate, 2) << 7;
	...
	TWIWrite((uint8_t)(A2M1));
	if (TWIGetStatus() != 0x28) TWIError();

	TWIWrite((uint8_t)(A2M2));
	if (TWIGetStatus() != 0x28) TWIError();

	TWIWrite((uint8_t)(A2M3));
	if (TWIGetStatus() != 0x28) TWIError();
